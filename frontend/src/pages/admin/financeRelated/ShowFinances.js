...
const ShowFinances = () => {
  ...
  useEffect(() => {
    ...
    if (currentUser?.schoolName) {
      dispatch(getUsers(currentUser.schoolName, "Finance"))
    }
  ...
  useEffect(() => {
    if (status === 'deleted') {
      ...
      setShowPopup(true)
      setMessage("Finance personnel deleted successfully")
      setLoader(false)
    }
  ...
  const deleteHandler = (id) => {
    setLoader(true)
    dispatch(deleteUser(id, "Finance"))
  }
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
              <Box>
                ...
                <Typography variant="h6" sx={{ color: 'rgba(255,255,255,0.8)', fontWeight: 400 }}>
                  Manage your school's finance team
                </Typography>
              </Box>
              <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                ...
                <Button
                  variant="contained"
                  startIcon={<AddIcon />}
                  onClick={() => navigate('/Admin/finances/add')}
                  sx={{
          ...
          {status === 'loading' ? (
            ...
            <Box>
              ...
              <StyledTableContainer component={Paper} sx={{ mb: 4 }}>
                <Table>
                  ...
                  <TableBody>
                    {safeUserDetails.map((finance, index) => (
                      <TableRow key={finance._id} sx={{ '&:hover': { background: 'rgba(102, 126, 234, 0.05)' } }}>
                        <TableCell>
                        ...
                        <TableCell>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            ...
                            <Typography variant="body2">{finance.email}</Typography>
                          </Box>
                        </TableCell>
                        <TableCell>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            <PhoneIcon sx={{ fontSize: 16, color: '#667eea' }} />
                            <Typography variant="body2">{finance.phone}</Typography>
                          </Box>
                        </TableCell>
                        <TableCell>
                          <Chip 
                        ...
                        <TableCell align="center">
                          <Tooltip title="Delete Finance Personnel">
                            <IconButton
                              onClick={() => deleteHandler(finance._id)}
                              disabled={loader}
                              sx={{
                                color: '#e74c3c',
                                '&:hover': {
              ...
              <Grid container spacing={3}>
                {safeUserDetails.map((finance, index) => (
                  <Grid item xs={12} sm={6} md={4} key={finance._id}>
                    <FinanceCard>
                      <CardContent sx={{ p: 3 }}>
            ...
            <Box sx={{ 
              ...
              <Typography variant="h5" sx={{ fontWeight: 600, color: '#2c3e50', mb: 2 }}>
                No Finance Personnel Found
              </Typography>
              <Typography variant="body1" sx={{ color: '#7f8c8d', mb: 3, maxWidth: '400px', mx: 'auto' }}>
                Get started by adding your first finance personnel to manage your school's financial operations.
              </Typography>
              <Button
                variant="contained"
                startIcon={<AddIcon />}
