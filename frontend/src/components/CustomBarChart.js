...
const CustomTooltipContent = ({ active, payload, dataKey }) => {
    if (active && payload && payload.length) {
        ...
        // Generic tooltip that works with any data structure
        return (
            <CustomTooltip>
                <TooltipMain>
                    {data.subject || data.name || 'N/A'}
                </TooltipMain>
                <TooltipText>
                    {data[dataKey]}{dataKey === 'performance' ? '%' : ''}
                </TooltipText>
            </CustomTooltip>
        );
    }
...
const CustomBarChart = ({ chartData, dataKey }) => {
    const distinctColors = generateDistinctColors(chartData.length);
